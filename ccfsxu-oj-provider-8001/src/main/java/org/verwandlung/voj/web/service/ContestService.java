	...
	public Map<String, String> getCodeSnippetOfContestProblem(Contest contest, long problemId, User contestant) {
		...
		if ( contest.getContestMode().equals("OI") && getContestStatus(contest) == Contest.CONTEST_STATUS.LIVE ) {
			ContestContestant cc = contestContestantMapper
					.getContestantOfContest(contest.getContestId(), contestant.getUid());	//这里获取这个竞赛中的这个问题
...
//		rankingContestants(contestants);
	...
	private Map<Long, Map<Long, Submission>> getSubmissionsGroupByContestant(
		...
		for ( ContestSubmission cs : contestSubmissions ) {
			...
			if ( !submissions.containsKey(contestantUid) ) {	//获取这个竞赛的所有提交记录
				submissions.put(contestantUid, new HashMap<>());
			}
			Map<Long, Submission> submissionsOfContestant = submissions.get(contestantUid);	//这个就相当于是一个指针

			if ( !override && submissions.get(contestantUid).containsKey(problemId) ) {
				continue;
			}
	 ...
	 * @param contestNotes - 竞赛说明
	 * @param startTime - 开始时间
	 * @param endTime - 结束时间
	 * @param mode - 竞赛模式（OI或者ACM）
	 * @param problems - 竞赛中的问题
	 * @return 竞赛创建的结果
	 */
	public Map<String, Object> createContest(String contestName,String contestNotes,
					Date startTime,Date endTime, String mode, String problems){
		Contest contest = new Contest(contestName,contestNotes,startTime,endTime,mode,problems);
	...
	private Map<String, ? extends Object> getContestCreationResult(Contest contest) {
		...
		result.put("isStartTimeEmpty",contest.getStartTime()==null);
		result.put("isEndTimeEmpty",contest.getEndTime()==null);
		result.put("isModeEmpty",contest.getContestMode().isEmpty());
		result.put("isProblemsEmpty",contest.getProblems().isEmpty());
		Boolean isSuccessful = !result.get("isContestNameEmpty") &&
				!result.get("isStartTimeEmpty") && !result.get("isEndTimeEmpty") &&
				!result.get("isModeEmpty") && !result.get("isProblemsEmpty");
		result.put("isSuccessful",isSuccessful);
		return result;
	}
	 ...
	 * 创建竞赛与提交记录的对应关系
	 * @param contest -竞赛
	 * @param submission -竞赛的提交记录（最后数据库中用的还是id）
	 * @return boolean
	 */
	public boolean createContestSubmission(Contest contest, Submission submission){
